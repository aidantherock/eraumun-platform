    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500 mb-6">This post does not exist or has been removed.</p>
      ...
      {post.cover_image_url ? (
        <div className="h-64 md:h-80 relative overflow-hidden">
          ...
          <div className="absolute bottom-0 left-0 right-0 px-6 py-8 max-w-4xl mx-auto">
            <span className="text-xs font-bold uppercase tracking-widest text-[#d4af62] mb-2 block">{post.category}</span>
            <h1 className="font-serif text-3xl md:text-4xl font-bold text-white leading-tight">{post.title}</h1>
          </div>
        </div>
      ) : (
        <div className="bg-gradient-to-br from-[#1e3a6e] via-[#162d58] to-[#0f2040] px-6 py-16">
          <div className="max-w-4xl mx-auto">
            <span className="text-xs font-bold uppercase tracking-widest text-[#d4af62] mb-3 block">{post.category}</span>
            <h1 className="font-serif text-3xl md:text-4xl font-bold text-white leading-tight">{post.title}</h1>
          </div>
        </div>
      )}
      ...
      <section className="px-6 py-12 bg-white">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="space-y-4">
            {post.content.split('\n').map((paragraph, i) => (
              paragraph.trim()
                ? <p key={i} className="text-gray-700 leading-relaxed text-base">{paragraph}</p>
                : <br key={i} />
            ))}
          </div>
